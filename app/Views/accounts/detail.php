...
<section class="space-y-2">
    <h1>Detail Akun</h1>
    <p class="max-w-[760px] font-serif text-[clamp(18px,1.35vw,20px)] leading-[1.45] text-[rgba(38,37,30,0.64)]">Ringkasan identitas akun, daftar subscription, status akses invite, serta histori perubahan usage.</p>
</section>
...
<?php foreach ($subscriptions as $subscription): ?>
    ...
    <section class="mt-6 <?= $cardBase ?> bg-surface400 space-y-3">
        <div class="flex flex-wrap items-center justify-between gap-2">
            ...
            <div class="font-mono text-[11px] leading-[1.55] tracking-[-0.01em] text-[rgba(38,37,30,0.76)]">Invite Expired: <?= esc($subscription['expired_at']) ?></div>
        </div>
        ...
        <form method="post" action="/subscriptions/<?= esc((string) $subscription['id']) ?>/update" class="space-y-3">
            <div class="grid gap-3 [grid-template-columns:repeat(auto-fit,minmax(220px,1fr))]">
                ...
                <label class="<?= $labelClass ?>">
                    Invite Expired
                    <input class="<?= $inputClass ?>" type="datetime-local" name="expired_at" required value="<?= esc(date('Y-m-d\\TH:i', strtotime($subscription['expired_at']))) ?>">
                </label>
            </div>
        ...
        <div class="grid gap-3 [grid-template-columns:repeat(auto-fit,minmax(220px,1fr))]">
            <?php foreach (['5h' => 'Usage 5 Jam', 'weekly' => 'Usage Mingguan'] as $type => $label): ?>
                    ...
                    <?php else: ?>
                        ...
                        <dialog id="usage-modal-<?= esc((string) $usage['id']) ?>" class="w-[min(540px,92vw)] rounded-lg border border-[rgba(38,37,30,0.1)] bg-surface400 p-0 shadow-[rgba(0,0,0,0.14)_0_28px_70px,rgba(0,0,0,0.1)_0_14px_32px]">
                            <form method="post" action="/usages/<?= esc((string) $usage['id']) ?>/update" class="space-y-3 p-4">
                                ...
                                <label class="<?= $labelClass ?>">
                                    Waktu Reset
                                    <input class="<?= $inputClass ?>" type="datetime-local" name="reset_at" required value="<?= esc(date('Y-m-d\\TH:i', strtotime((string) $usage['reset_at']))) ?>">
                                </label>
